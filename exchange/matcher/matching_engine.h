...
namespace Exchange {
	...
	public:
		...
		// Called to process a client request read from the lock free queue sent by 
		// the order server.
		...
		// Write market data update to the lock free queue for the market data publisher 
		// to consume.
		...
		// Main loop for this thread - processes incoming client requests which in turn 
		// generates client responses and market updates.
		auto run() noexcept {
