...
namespace Exchange {
	...
	void SnapshotSynthesizer::start() {
		run_ = true;
		ASSERT(Common::createAndStartThread(
			-1, "Exchange/SnapshotSynthesizer", 
			[this]() { run(); }
		) != nullptr, "Failed to start SnapshotSynthesizer thread.");
	}
	...
	auto SnapshotSynthesizer::addToSnapshot(const MDPMarketUpdate *market_update) {
		...
		ASSERT(
			market_update->seq_num_ == last_inc_seq_num_ + 1, 
	...
	void SnapshotSynthesizer::run() {
		...
		while (run_) {
			for (
				auto market_update = snapshot_md_updates_->getNextToRead(); 
