...
namespace Common {
    // Defining some logger structuress
    ...
    class Logger final {
    public:
        /// Consumes from the lock free queue of log entries and writes to the 
        // output log file.
        ...
        explicit Logger(const std::string &file_name)
            ...
            logger_thread_ = createAndStartThread(
                -1, "Common/Logger " + file_name_, 
                [this]() { flushQueue(); }
            );
        ...
        // Parse the format string, substitute % with the variable number 
        // of arguments passed and write the string to the lock free queue.
            ...
            while (*s) 
            {
                if (*s == '%') {
                    if (UNLIKELY(*(s + 1) == '%')) { // to allow %% -> % escape character.
                        ...
                        // substitute % with the value specified in the arguments.
                        pushValue(value); 

                        // pop an argument and call self recursively.
                        log(s + 1, args...); 
                        return;
                    }
                }
    ...
    private:
        ...
        /// Lock free queue of log elements from main logging thread to background 
        // formatting and disk writer thread.
